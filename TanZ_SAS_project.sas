...
DATE: Tuesday, January 13, 2015     TIME: 2:54:33 PM
...
/* -------------------------------------------------------------------
   ...
   Generated on: Tuesday, January 13, 2015 at 2:54:13 PM
...
/* -------------------------------------------------------------------
   ...
   Generated on: Tuesday, January 13, 2015 at 2:54:14 PM
...
/* -------------------------------------------------------------------
   ...
   Generated on: Tuesday, January 13, 2015 at 2:54:15 PM
...
PROC SQL;
	CREATE VIEW WORK.SORTTempTableSorted AS
		SELECT T.COUNT_of_DUPERSID, T."Count DUPERSID CAT"n
	FROM WORK.CAT_COUNT_RECODED_MEPS_MERGED_CO as T
;
QUIT;
TITLE;
...
PROC FREQ DATA = WORK.SORTTempTableSorted
	ORDER=INTERNAL
;
	TABLES COUNT_of_DUPERSID * "Count DUPERSID CAT"n /
		NOROW
		NOCOL
		NOPERCENT
		MISSPRINT
...
%_eg_conditional_dropds(WORK.SORT);
/* -------------------------------------------------------------------
   Sort data set Local:ZDATA.MEPS_MERGED_COUNT
   ------------------------------------------------------------------- */

PROC SQL;
	CREATE VIEW WORK.SORT AS
		SELECT T.COUNT_of_DUPERSID, T.DUPERSID1, T.DUPERSID, T.DUPERSID2
	FROM ZDATA.MEPS_MERGED_COUNT as T
;
QUIT;
...
;
	TABLES COUNT_of_DUPERSID /  SCORES=TABLE;
	TABLES DUPERSID1 /  SCORES=TABLE;
	TABLES DUPERSID /  SCORES=TABLE;
	TABLES DUPERSID2 /  SCORES=TABLE;
RUN;
/* -------------------------------------------------------------------
   End of task code.
   ------------------------------------------------------------------- */
